<dl>
	...
	<dd>
		<input type="number" id="updatedays" name="updatedays" value="{$days}" class="medium" min="0" />
...
{foreach from=$configPreset key=name item=track}
	{if $name != 'username' && $name != 'userID' && $name != 'isActive' && $name != 'days' && $name != 'time' && $name != 'trackerID'}
		...
		<dl>
			...
			<dd>
				<label><input type="checkbox" id="update{$name}" name="update{$name}" value={$track}{if $track == 1} checked="checked"{/if} /> {lang}wcf.uztracker.config.{$name}{/lang}</label>
			</dd>
		</dl>
